...
describe ClearancingService do
  ...
  describe "::process_input" do
    context "total success" do
      ...
      let(:inputs) {items.map(&:id)}

      before do
        @clearancing_status = clearancing_service.process_input(inputs)
      end
      ...
      it "adds all the items to the batch" do
        expect(@clearancing_status.clearance_batch.items.pluck(:id).sort).to eq(@clearancing_status.item_ids_to_clearance.sort)
      end
    ...
    context "partial success" do
      ...
      let(:inputs){ valid_items.map(&:id) + invalid_items }
